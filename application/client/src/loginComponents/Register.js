...
class Register extends React.Component {
    state = { username: null, password: null, reenteredPassword: null, firstName: null, lastName: null, email: null, imageURL: null };
        ...
        return(
            <div>
                <h1>New user registration</h1>
                        ...
                        imageURL(optional): <input type = "text" name = "image URL" placeholder = "imageURL" onChange = {(event) => this.setState({ imageURL: event.target.value})}/>
                        <Link to = "/"><button className = "btn" onClick = {() => this.props.addNewUserCustom(this.state.username, this.state.password, this.state.firstName, this.state.lastName, this.state.email, this.state.imageURL )}>Register</button></Link>
